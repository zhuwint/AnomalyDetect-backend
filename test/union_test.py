...
def test_union():
    ...
    while(p1 < len(humidity) and p2 < len(temperature)):
        ...
        if humidity.index[p1] < temperature.index[p2]:
            point = build_point("3", "CC11", "humidity_air", humidity.iloc[p1].value, humidity.index[p1])
            p1 += 1
        else:
            point = build_point("2", "CC11", "temperature_air", temperature.iloc[p1].value, temperature.index[p2])
            p2 += 1
        write(point)
        count += 1
        # if count == 100:
